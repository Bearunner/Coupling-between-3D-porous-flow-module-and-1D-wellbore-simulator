...
#include "PorousFlowMassTimeDerivativeS2.h"
...
registerMooseObject("PorousFlowApp", PorousFlowMassTimeDerivativeS2);

InputParameters
PorousFlowMassTimeDerivativeS2::validParams()
{
...
PorousFlowMassTimeDerivativeS2::PorousFlowMassTimeDerivativeS2(const InputParameters & parameters)
  : TimeKernel(parameters),
    ...
    // _new_C2_old(getMaterialPropertyOld<std::vector<std::vector<Real>>>("new_C2"))
    // _new_C2_old(getMaterialPropertyOld<Real>("new_C2"))
...
Real
PorousFlowMassTimeDerivativeS2::computeQpResidual()
{
  ...
  for (unsigned ph = 0; ph < _num_phases; ++ph)
  {
    ...
    mass_old +=
        dens_old * _fluid_saturation_nodal_old[_i][ph] * (_mass_frac_old[_i][ph][_fluid_component-1] * (1.0 - tracer_indicator) + _mass_frac_old[_i][ph][_fluid_component]);
    // mass_old +=
    //     dens_old * _fluid_saturation_nodal_old[_i][ph] * _new_C2_old[_i][ph][_fluid_component];
    // mass_old +=  dens_old * _new_C2_old[_qp];
  }
...
Real
PorousFlowMassTimeDerivativeS2::computeQpJacobian()
{
...
Real
PorousFlowMassTimeDerivativeS2::computeQpOffDiagJacobian(unsigned int jvar)
{
...
Real
PorousFlowMassTimeDerivativeS2::computeQpJac(unsigned int pvar)
{
