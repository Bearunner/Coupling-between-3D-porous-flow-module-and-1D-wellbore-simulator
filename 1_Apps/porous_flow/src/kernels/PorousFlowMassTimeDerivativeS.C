...
#include "PorousFlowMassTimeDerivativeS.h"
...
registerMooseObject("PorousFlowApp", PorousFlowMassTimeDerivativeS);

InputParameters
PorousFlowMassTimeDerivativeS::validParams()
{
...
PorousFlowMassTimeDerivativeS::PorousFlowMassTimeDerivativeS(const InputParameters & parameters)
  : TimeKernel(parameters),
    ...
    _T_old(getMaterialPropertyOld<Real>("PorousFlow_temperature_nodal"))
    // _new_C1_old(getMaterialPropertyOld<std::vector<std::vector<Real>>>("new_C1"))
    // _new_C1_old(getMaterialPropertyOld<Real>("new_C1_nodal"))
...
Real
PorousFlowMassTimeDerivativeS::computeQpResidual()
{
  ...
  for (unsigned ph = 0; ph < _num_phases; ++ph)
  {
    ...
    // mass_old +=
    //     dens_old * _fluid_saturation_nodal_old[_i][ph] * _mass_frac_old[_i][ph][_fluid_component];//Original
    mass_old +=
        dens_old * _fluid_saturation_nodal_old[_i][ph] * tracer_indicator * _mass_frac_old[_i][ph][_fluid_component];//Original
    // mass_old +=
    //     dens_old * _fluid_saturation_nodal_old[_i][ph] * _new_C1_old[_i][ph][_fluid_component];
    // mass_old += dens_old * _new_C1_old[_i];
  }
  const Real strain = (_has_total_strain ? (*_total_strain_old)[_qp].trace() : 0.0);

  return _test[_i][_qp] * (1.0 + strain) * (_porosity[_i] * mass - _porosity_old[_i] * mass_old) / _dt;//Original
  // return _scale_factor[_qp] * _TimeKernelS[_qp] * test * (_u[_qp] - _new_C1_old[_qp]) / _dt;//example from Tiger
  // return _test[_i][_qp] * (1.0 + strain) * (_porosity[_i] * mass - _porosity_old[_i] * _new_C1_old[_qp]) / _dt;
}

Real
PorousFlowMassTimeDerivativeS::computeQpJacobian()
{
...
Real
PorousFlowMassTimeDerivativeS::computeQpOffDiagJacobian(unsigned int jvar)
{
...
Real
PorousFlowMassTimeDerivativeS::computeQpJac(unsigned int pvar)
{
