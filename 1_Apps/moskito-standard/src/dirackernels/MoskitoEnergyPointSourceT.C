...
#include "MoskitoEnergyPointSourceT.h"
#include "FEProblemBase.h"

registerMooseObject("MoskitoApp", MoskitoEnergyPointSourceT);


InputParameters
MoskitoEnergyPointSourceT::validParams()
{
  InputParameters params = DiracKernel::validParams();
  params.addParam<Real>("energy_flux",0.0,
        "The constant energy flow rate at this point (well bottom) in J/s "
        "(negative is injection, positive is production)");
  params.addParam<FunctionName>("energy_flux_function", "The energy flow rate as a "
        "function of time at this point (well bottom) in J/s (negative-valued "
...
MoskitoEnergyPointSourceT::MoskitoEnergyPointSourceT(
    const InputParameters & parameters)
  : DiracKernel(parameters),
    _energy_flux(getParam<Real>("energy_flux")),
...
{
  _energy_flux_function = isParamValid("energy_flux_function") ?
                        &getFunction("energy_flux_function") : NULL;

  // Sanity check to ensure that the end_time is greater than the start_time
  if (_end_time <= _start_time)
    mooseError("Start time for MoskitoEnergyPointSourceT is ",_start_time,
                " but it must be less than end time ",_end_time);
}

void
MoskitoEnergyPointSourceT::addPoints()
{
  addPoint(_p);
}

Real
MoskitoEnergyPointSourceT::computeQpResidual()
{
  // to make injection negative and compatible as a sourceterm
  Real factor = -1.0;

  if (isParamValid("energy_flux_function"))
      factor *= _energy_flux_function->value(_t, Point());
  else
  {
    /**
     ...
     * end) interval, then the  energy_flux is scaled so that the total energy added
  ...
  // Negative sign to make a positive energy_flux as a source
  // return -_test[_i][_qp] * factor;
  return -_test[_i][_qp] * factor / _area[_qp];
