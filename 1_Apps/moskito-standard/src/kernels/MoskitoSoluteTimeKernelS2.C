...
#include "MoskitoSoluteTimeKernelS2.h"

registerMooseObject("MoskitoApp", MoskitoSoluteTimeKernelS2);

InputParameters
MoskitoSoluteTimeKernelS2::validParams()
{
  ...
  params.addCoupledVar("original_tracer", "original_tracer");

  return params;
}

MoskitoSoluteTimeKernelS2::MoskitoSoluteTimeKernelS2(const InputParameters & parameters)
  : TimeDerivative(parameters),
    _T_old(coupledValueOld("temperature")),
    _T_threshold(getParam<Real>("temperature_threshold")),
    _u_old(valueOld()),
    _original_tracer(coupledValue("original_tracer")),
    _scale_factor(getMaterialProperty<Real>("scale_factor")),
    _TimeKernelS(getMaterialProperty<Real>("TimeKernel_S")),
    // _new_C6_old(getMaterialPropertyOld<Real>("new_C6")),
...
Real
MoskitoSoluteTimeKernelS2::computeQpResidual()
{
  ...
  return _scale_factor[_qp] * _TimeKernelS[_qp] * test * (_u[_qp] - (_original_tracer[_qp] * (1.0 - tracer_indicator) + _u_old[_qp])) / _dt;
  // return _scale_factor[_qp] * _TimeKernelS[_qp] * test * _u_dot[_qp]; ## initial

}

Real
MoskitoSoluteTimeKernelS2::computeQpJacobian()
{
  Real test = 0.0, j = 0.0;
