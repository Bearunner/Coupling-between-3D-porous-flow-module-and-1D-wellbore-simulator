...
//  by MOOSE.
#include "MoskitoDictator.h"
#include "NonlinearSystem.h"

registerMooseObject("MoskitoApp", MoskitoDictator);

InputParameters
MoskitoDictator::validParams()
{
  InputParameters params = GeneralUserObject::validParams();
  params.addClassDescription("Holds information on the Moskito variable names");
  params.addRequiredCoupledVar("porous_flow_vars",
                               "List of primary variables that are used in the Moskito "
...
MoskitoDictator::MoskitoDictator(const InputParameters & parameters)
    ...
    _fe_type(0)
{
  ...
  for (unsigned int i = 0; i < _num_variables; ++i)
    if (_moose_var_num[i] < _pf_var_num.size())
      ...
      // them)
      mooseError("MoskitoDictator: AuxVariables variables must not be coupled into the Dictator "
                 "for this is against specification #1984.  Variable number ",
                 i,
                 " is an AuxVariable.");

  if ((_num_phases > 0) && (_aqueous_phase_number >= _num_phases))
    mooseError("MoskitoDictator: The aqueous phase number must be less than the number of fluid "
...
unsigned int
MoskitoDictator::numVariables() const
{
  return _num_variables;
}

unsigned int
MoskitoDictator::numPhases() const
{
  return _num_phases;
}

unsigned int
MoskitoDictator::numComponents() const
{
  return _num_components;
}

unsigned int
MoskitoDictator::numAqueousEquilibrium() const
{
  return _num_aqueous_equilibrium;
}

unsigned int
MoskitoDictator::numAqueousKinetic() const
{
  return _num_aqueous_kinetic;
}

unsigned int
MoskitoDictator::aqueousPhaseNumber() const
{
  return _aqueous_phase_number;
}

unsigned int
MoskitoDictator::MoskitoVariableNum(unsigned int moose_var_num) const
{
  if (moose_var_num >= _pf_var_num.size() || _pf_var_num[moose_var_num] == _num_variables)
    mooseError("The Dictator proclaims that the moose variable with number ",
               moose_var_num,
               " is not a Moskito variable.  Exiting with error code 1984.");
  return _pf_var_num[moose_var_num];
}

unsigned int
MoskitoDictator::mooseVariableNum(unsigned int porous_flow_var_num) const
{
  if (porous_flow_var_num >= _num_variables)
    mooseError("The Dictator proclaims that there is no such Moskito variable with number ",
               porous_flow_var_num,
...
bool
MoskitoDictator::isMoskitoVariable(unsigned int moose_var_num) const
{
  return !notMoskitoVariable(moose_var_num);
}

bool
MoskitoDictator::notMoskitoVariable(unsigned int moose_var_num) const
{
  return moose_var_num >= _pf_var_num.size() || _pf_var_num[moose_var_num] == _num_variables;
}

bool
MoskitoDictator::consistentFEType() const
{
  return _consistent_fe_type;
}

FEType
MoskitoDictator::feType() const
{
  return _fe_type;
}
