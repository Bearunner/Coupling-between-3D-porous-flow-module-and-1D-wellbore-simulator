...
#include "MoskitoSoluteMassChangeRate.h"

registerMooseObject("MoskitoApp", MoskitoSoluteMassChangeRate);

InputParameters
MoskitoSoluteMassChangeRate::validParams()
{
  InputParameters params = AuxKernel::validParams();
  // params.addCoupledVar("solute_name", "Solute");
  params.addRequiredCoupledVar("solute", "solute name");
...
MoskitoSoluteMassChangeRate::MoskitoSoluteMassChangeRate(const InputParameters & parameters)
  : AuxKernel(parameters),
    // _C_old(coupledValueOld("solute_name")),
    // _C(coupledValue("solute_name")),
    _C_dot(coupledDot("solute"))
...
Real
MoskitoSoluteMassChangeRate::computeValue()
{
  ...
  // return test * (_C[_qp] - _C_old[_qp]) / _dt;
  return _C_dot[_qp];

}

// Real
// MoskitoSoluteMassChangeRate::computeQpJacobian()
// {
//   Real test = 0.0, j = 0.0;
//
//   if (_SUPG_ind[_qp])
